...
# ---------- تهيئة قاعدة البيانات (تنشئ الجداول مرة واحدة فقط) ----------
@st.cache_resource
def init_db():
...
# ---------- دالة مساعدة لفتح اتصال جديد ----------
def get_conn():
    """فتح اتصال جديد بقاعدة البيانات"""
    return sqlite3.connect('rentals.db')
        ...
        try:
            ...
            if tenants:
                ...
                selected_id = st.selectbox(
                    "اختر مستأجر",
                    tenant_ids,
                    format_func=lambda x: next(t[1] for t in tenants if t[0] == x)
                )
                if selected_id:
        ...
        with st.form("add_tenant"):
            ...
            if submit:
                if name:
                    ...
                    try:
                        cur.execute(
                            "INSERT INTO tenants (name, phone, region, notes) VALUES (?,?,?,?)",
        ...
        if not tenants or not properties:
            ...
            with st.form("add_contract"):
                ...
                property_id = st.selectbox(
                    ...
                    format_func=lambda x: next(p[1] for p in properties if p[0] == x)
                )
                ...
                if submit:
                    ...
                    try:
                        ...
                        # جدولة الدفعات باستخدام relativedelta
                        freq_map = {
                        ...
                        step = freq_map[payment_frequency]
                        current = start_date
            ...
            if payment_id:
                ...
                with st.form("pay_form"):
                    ...
                    if st.form_submit_button("تسجيل"):
                        ...
                        cur.execute(
                            "INSERT INTO receipts (tenant_id, amount, receipt_date, payment_method) VALUES (?,?,?,?)",
                            (pay[2], amount_paid, pay_date.isoformat(), method)
                        )
                        conn.commit()
    ...
    if report == "كشف حساب مستأجر":
        ...
        try:
            ...
            if tenants:
                ...
                cur.execute(
                    "SELECT due_date, amount, paid_amount, (amount - paid_amount), status FROM payments WHERE tenant_id = ? ORDER BY due_date",
                    (tenant_id,)
                )
                pays = cur.fetchall()
                if pays:
                    ...
                    st.download_button(
                        ...
                        file_name=f"كشف_حساب_{tenant_name}.xlsx"
                    )
                else:
                    st.info("لا توجد دفعات")
            else:
        ...
        try:
            cur.execute(
                "SELECT receipt_date, tenant_id, amount, payment_method FROM receipts WHERE receipt_date BETWEEN ? AND ?",
